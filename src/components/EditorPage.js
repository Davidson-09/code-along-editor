...
export default function EditorPage() {
  ...
  useEffect(() => {
    ...
    if (selectedFolderPath !== null) {
      const getFileTree = async () => {
        try {
          const tree = await window.electronApi.filesApi.buildFileTree(
            selectedFolderPath
          );
          return tree;
        } catch (e) {
        ...
        if (currentFile) {
          if (currentFile.fileContent !== editorValue) {
            try {
              ...
              await window.electronApi.filesApi.saveFileChanges({
                filePath,
  ...
  const handleCreateNewFile = async () => {
    try {
      // create the file
      const result = await window.electronApi.filesApi.saveNewFile(
        selectedFolderPath
      );
  ...
  return (
    <div>
      {editorIsVisible ? (
        <div>
          {fileSelectorIsVisible && (
            <FileSelector
              fileTree={fileTree}
              setIsVisble={setFileSelectorIsVisible}
              setData={setEditorValue}
              setReload={setReload}
            />
          )}
          <div className="p-3 flex space-x-4 base-color">
            ...
            <p className="flex-1 text-center">
              {currentFileName}{" "}
              {currentFile && editorValue !== currentFile.fileContent && (
                <span>*</span>
              )}
            </p>
          </div>
          {editorValue !== null ? (
            ...
            <div className="h-[85vh] md:h-[93vh] w-full bg-gray-50 flex justify-center items-center flex-col space-y-4">
              <button
                onClick={handleCreateNewFile}
                className="border-gray-300 p-3 rounded text-sm base-color"
              >
                Create New File
              </button>
              <div>
                <p className="font-bold text-[#0C2D57] text-center mb-2">
                  Useful Tips
                </p>
        ...
        <div className="flex h-screen w-full bg-gray-100 flex-col justify-center items-center space-y-4">
          ...
          <button
            className="py-3 px-4 base-color rounded-lg font-bold"
